 ...
 * Runs in Electron main process for file system access
 */
export class CodingAssistant extends LLMHelper {
  ...
  private documentEngine: DocumentTemplateEngine | null = null
  ...
  constructor(apiKey?: string, useOllama?: boolean, ollamaModel?: string, ollamaUrl?: string) {
    ...
    // Initialize document template engine
    this.documentEngine = new DocumentTemplateEngine(this)
  }
  ...
  async generateDocument(request: DocumentGenerationRequest): Promise<GeneratedDocument> {
    if (!this.documentEngine) {
      throw new Error('Document template engine not initialized')
    }

    if (!this.projectContext) {
      throw new Error('No project context available - initialize session first')
    }

    // Ensure project context is set in the request
    const enrichedRequest: DocumentGenerationRequest = {
      ...request,
      projectContext: this.projectContext
    }

    // Validate the request
    const validation = this.documentEngine.validateRequest(enrichedRequest)
    if (!validation.valid) {
      throw new Error(`Document request validation failed: ${validation.errors.join(', ')}`)
    }

    console.log(`[CodingAssistant] Generating ${request.type} document for project: ${this.projectContext.name}`)

    try {
      const document = await this.documentEngine.generateDocument(enrichedRequest)

      // Add to session notes
      if (this.currentSession) {
        const note = `Generated ${request.type.toUpperCase()} document: ${document.title}`
      ...
      console.error(`[CodingAssistant] Error generating ${request.type} document:`, error)
      throw error
    }
  }
  ...
  getAvailableDocumentTypes(): Array<{ type: string; name: string; description: string }> {
    ...
    return this.documentEngine.getAvailableDocumentTypes()
  }
  ...
  async exportDocument(
    ...
    try {
      const exportedContent = await this.documentEngine.exportDocument(document, format)

      // Save to file if output path provided
      if (outputPath) {
        const fs = require('fs')
        fs.writeFileSync(outputPath, exportedContent)
        console.log(`[CodingAssistant] Document exported to: ${outputPath}`)
      }

      return exportedContent
    } catch (error) {
      console.error('[CodingAssistant] Error exporting document:', error)
      throw error
    }
  }

  /**
   * Quick document generation with common presets
   */
  async generatePRD(requirements: string[], additionalContext?: string): Promise<GeneratedDocument> {
    if (!this.projectContext) {
    ...
    return this.generateDocument({
      ...
      projectContext: this.projectContext,
      requirements,
      targetAudience: 'Product team and developers',
      additionalContext
    })
  }

  async generateTechnicalSpec(requirements: string[]): Promise<GeneratedDocument> {
    if (!this.projectContext) {
      throw new Error('No project context available')
    }
  ...
  async generateUserStories(requirements: string[], targetAudience?: string): Promise<GeneratedDocument> {
    if (!this.projectContext) {
      throw new Error('No project context available')
    }
    ...
    return this.generateDocument({
      ...
      projectContext: this.projectContext,
      requirements,
      targetAudience: targetAudience || 'End users and stakeholders'
    })
  }
  ...
  async generateAPIDocumentation(): Promise<GeneratedDocument> {
    ...
    return this.generateDocument({
      type: 'api-doc',
      projectContext: this.projectContext,
      requirements: ['Document all API endpoints and functionality'],
      targetAudience: 'API consumers and developers'
    })
  }

  async generateArchitectureDecision(requirements: string[]): Promise<GeneratedDocument> {
    if (!this.projectContext) {
      throw new Error('No project context available')
    }
    ...
    return this.generateDocument({
      ...
      projectContext: this.projectContext,
      requirements,
      targetAudience: 'Architecture team and developers'
    })
  }
}
